...
import org.jetbrains.plugins.scala.lang.psi.api.expr.{ExpectedTypes, ScExpression}
...
object CacheWithRecursionGuard {
    ...
    try {
      ...
      if (fromLocalCache != null) {
        ...
        val (result, shouldCache) = try {
          ...
          val shouldCache = stackStamp.mayCacheNow() && canCacheForExpr(element)
          (result, shouldCache)
        } finally {
  ...
  private def canCacheForExpr(e: PsiElement): Boolean = e match {
    case expr: ScExpression => ExpectedTypes.canCacheTypeFor(expr)
    case _                  => true
  }

  // Code generated by @CachedWithRecursionGuard macro annotation, method with parameters
  def cacheWithRecursionGuardN[T <: Product, R](id: String, name: String, element: PsiElement, defaultValue: => R, dependencyItem: => AnyRef, v: T, f: => R): R = {
    val data = v
