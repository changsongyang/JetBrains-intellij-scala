...
private[evaluation] trait ScalaEvaluatorBuilderUtil {
  ...
  def importedQualifierEvaluator(ref: ScReference, resolveResult: ScalaResolveResult): Evaluator = {
    val message = DebuggerBundle.message("cannot.evaluate.imported.reference")
  ...
  def localMethodEvaluator(fun: ScFunctionDefinition, argEvaluators: Seq[Evaluator]): Evaluator = {
    ...
    val message = DebuggerBundle.message("cannot.evaluate.local.method")
  ...
  def syntheticFunctionEvaluator(synth: ScSyntheticFunction,
    ...
    def unaryEval(operatorName: String, function: Evaluator => Evaluator): Evaluator = {
      if (argEvaluators.isEmpty) {
        ...
        function(eval)
      } else throw EvaluationException(DebuggerBundle.message("wrong.number.of.arguments", operatorName))
    }
    ...
    def extractTypeArgumentForSyntheticMethod(methodName: String): ScType = {
      def missingTypeArgument() =
        throw EvaluationException(DebuggerBundle.message("missing.type.argument.synthetic.method", methodName))
    ...
    name match {
      ...
      case "synchronized" =>
        throw EvaluationException(DebuggerBundle.message("synchronized.statement.is.not.supported"))
      case _ =>
        throw EvaluationException(DebuggerBundle.message("cannot.evaluate.synthetic.method", name))
    }
  }
  ...
  def arrayMethodEvaluator(name: String, qual: Option[ScExpression], argEvaluators: Seq[Evaluator]): Evaluator = {
    val qualEval = qual match {
      ...
      case None => throw EvaluationException(DebuggerBundle.message("array.instance.is.not.found", name))
    }
    def message = DebuggerBundle.message("wrong.number.of.arguments", s"Array.$name")
    name match {
      case "apply" =>
      ...
      case _ =>
        throw EvaluationException(DebuggerBundle.message("array.method.not.supported"))
    }
  }
          ...
          case ScalaResolveResult(elem, _) =>
            ...
            clazz match {
              ...
              case _: ScObject => //todo: It can cover many cases!
                throw EvaluationException(DebuggerBundle.message("implicit.parameters.from.dependent.objects"))
    ...
    def addForNextClause(previousClausesEvaluators: Seq[Evaluator], clause: ScParameterClause): Seq[Evaluator] = {
      ...
      previousClausesEvaluators ++ clause.effectiveParameters.map {
        param =>
          ...
          if (p.isByName) throw new NeedCompilationException(DebuggerBundle.message("method.with.by-name.parameters"))

          val evaluator =
            if (p.isRepeated) repeatedArgEvaluator(exprsForP, p.expectedType, call)
            else if (exprsForP.size > 1) throw EvaluationException(DebuggerBundle.message("wrong.number.of.expressions"))
            ...
            else if (param.isImplicit) implicitArgEvaluator(fun, param, call)
            else if (p.isDefault) {
            ...
            else throw EvaluationException(DebuggerBundle.message("cannot.evaluate.parameter", p.name))
  ...
  def functionEvaluator(qualOption: Option[ScExpression], ref: ScReferenceExpression,
    ...
    resolved match {
      case Some(r) if r.tuplingUsed => throw EvaluationException(DebuggerBundle.message("tupling.not.supported"))
      case None => throw EvaluationException(DebuggerBundle.message("cannot.evaluate.method", funName))
  ...
  def methodCallEvaluator(call: ScExpression, arguments: Seq[ScExpression], matchedParameters: Map[Parameter, Seq[ScExpression]]): Evaluator = {
    val ref = call match {
      ...
      case _ => throw EvaluationException(DebuggerBundle.message("cannot.evaluate.method", call.getText))
    }
  ...
  def byNameParamEvaluator(ref: ScReferenceExpression, p: ScParameter, computeValue: Boolean): Evaluator =  {
    val paramEval = p match {
      ...
      case _: ScParameter if p.isCallByNameParameter =>
        calcLocal(p)
      case _ => throw EvaluationException(DebuggerBundle.message("by.name.parameter.expected"))
    }
  ...
  private def calcLocal(named: PsiNamedElement): Evaluator = {
      ...
      case param: ScParameter =>
        param.owner match {
          ...
          case _ =>
            ...
            throw EvaluationException(DebuggerBundle.message("cannot.evaluate.parameter", param.name))
        }
    ...
    containingClass match {
      ...
      case _ =>
        val fieldEval = withOuterFieldEvaluator(containingClass, name, DebuggerBundle.message("cannot.evaluate.local.variable", name))
  ...
  def evaluatorForReferenceWithoutParameters(qualifier: Option[ScExpression],
    ...
    def calcLocalObject(obj: ScObject) = {
      ...
      if (containingClass == contextClass) {
        ...
        val fieldEval = withOuterFieldEvaluator(containingClass, name, DebuggerBundle.message("cannot.evaluate.local.object", name))
        fieldEval
      }
    }
    ...
    resolve match {
      ...
      case cp: ScClassParameter if !cp.isClassMember =>
        ...
        new ErrorWrapperEvaluator(duplex, DebuggerBundle.message("constructor.param.inaccessible.outside.of.constructor", cp.name))
  ...
  def patternEvaluator(caseCl: ScCaseClause, namedElement: PsiNamedElement): Evaluator = {
    ...
    if (caseCl.getParent != null) {
      ...
      if (pattern.isEmpty) throw EvaluationException(DebuggerBundle.message("cannot.find.pattern"))
      caseCl.getParent.getParent match {
        case matchStmt: ScMatch if namedElement.isInstanceOf[ScPattern] =>
          val expr = matchStmt.expression match {
            case None => throw EvaluationException(DebuggerBundle.message("cannot.find.expression.of.match"))
            case Some(e) => e
          }
    ...
    } else throw EvaluationException(DebuggerBundle.message("invalid.case.clause"))
  }

  def assignmentEvaluator(stmt: ScAssignment): Evaluator = {
    val message = DebuggerBundle.message("assignent.without.expression")
  ...
  def evaluateSubpatternFromPattern(exprEval: Evaluator, pattern: ScPattern, subPattern: ScPattern): Evaluator = {
    def evaluateConstructorOrInfix(exprEval: Evaluator, ref: ScStableCodeReference, pattern: ScPattern, nextPatternIndex: Int): Evaluator = {
      ref.resolve() match {
        case fun: ScFunctionDefinition =>
          ...
          val newEval =
            if (funName == "unapply") {
              ...
              else throw EvaluationException(DebuggerBundle.message("unapply.without.arguments"))
        ...
        case _ => throw EvaluationException(DebuggerBundle.message("pattern.doesnot.resolves.to.unapply", ref.refName))
      }
    }
    ...
    templ.extendsBlock.templateParents match {
      case Some(parents) =>
        if (parents.typeElements.length != 1) {
          throw new NeedCompilationException(DebuggerBundle.message("anon.classes.not.supported"))
        }
            ...
            constrInvocation.typeElement.calcType.extractClass match {
              case Some(clazz) if clazz.qualifiedName == "scala.Array" =>
                def unspecifiedParameters =
                  throw EvaluationException(DebuggerBundle.message("array.constructor.unspecified.parameters"))
                def tooManyArguments =
                  throw EvaluationException(DebuggerBundle.message("array.constructor.too.many.arguments"))
              ...
              case Some(clazz) if isScalaSynthetic(clazz) =>
                ...
                if (clazz.qualifiedName == "scala.AnyRef") {
                  ...
                  throw EvaluationException(
                    DebuggerBundle.message("new.synthetic.instantiation", modifier, clazz.name))
                }
              case Some(clazz) =>
              ...
              case _ => throw EvaluationException(DebuggerBundle.message("new.expression.without.class.reference"))
            }

          case None => throw EvaluationException(DebuggerBundle.message("new.expression.without.constructor.call"))
        }
      case _ => throw EvaluationException(DebuggerBundle.message("new.expression.without.template.parents"))
    }
  }
  ...
  private def constructorArgumentsEvaluators(constrInvocation: ScConstructorInvocation): Seq[Evaluator] = {
    val constrDef = constrInvocation.reference match {
      ...
      case _ => throw EvaluationException(DebuggerBundle.message("could.not.resolve.constructor"))
    }
    ...
    constrDef match {
      case scMethod: ScMethodLike =>
        ...
        val implicitParams = scMethod.parameterList.params.filter(_.isImplicit)
  ...
  def ifStmtEvaluator(stmt: ScIf): Evaluator = {
    val condEvaluator = stmt.condition match {
      ...
      case None => throw EvaluationException(DebuggerBundle.message("if.statement.without.condition"))
    }
    val ifBranch = stmt.thenExpression match {
      case Some(th) => evaluatorFor(th)
      case None => throw EvaluationException(DebuggerBundle.message("if.statement.without.if.branch"))
    }
  ...
  def whileStmtEvaluator(ws: ScWhile): Evaluator = {
    val condEvaluator = ws.condition match {
      case Some(cond) => evaluatorFor(cond)
      case None => throw EvaluationException(DebuggerBundle.message("while.statement.without.condition"))
    }
    val iterationEvaluator = ws.expression match {
      case Some(body) => evaluatorFor(body)
      case None => throw EvaluationException(DebuggerBundle.message("while.statement.without.body"))
    }
  ...
  def doStmtEvaluator(doSt: ScDo): Evaluator = {
    val condEvaluator = doSt.condition match {
      case Some(cond) => evaluatorFor(cond)
      case None =>
        throw EvaluationException(DebuggerBundle.message("do.statement.without.condition"))
    }
    val bodyEvaluator = doSt.body match {
      case Some(body) => evaluatorFor(body)
      case None =>
        throw EvaluationException(DebuggerBundle.message("do.statement.without.body"))
    }
      ...
      case hasDeepestInvokedReference(ScReferenceExpression.withQualifier(implicitlyConvertedTo(expr))) =>
        ...
        copy match {
          case hasDeepestInvokedReference(ScReferenceExpression.withQualifier(q)) =>
            ...
            evaluatorFor(copy)
          case _ =>
            val message = DebuggerBundle.message("method.call.implicitly.converted.qualifier", methodCall.getText)
            throw EvaluationException(message)
        }
      case _ =>
  ...
  def variableDefinitionEvaluator(vd: ScVariableDefinition): Evaluator = {
    vd.expr match {
      case None => throw EvaluationException(DebuggerBundle.message("variable.definition.needs.right.hand.side", vd.getText))
  ...
  def patternDefinitionEvaluator(pd: ScPatternDefinition)
                                : Evaluator = {
    pd.expr match {
      case None => throw EvaluationException(DebuggerBundle.message("value.definition.needs.right.hand.side", pd.getText))
